            ...
            <div class="slider">
              <div class="thumbnail">
               @foreach ($products->galleries as $item)
                  ...
                  <div class="px-2">
                    <div
                      class="item {{ $loop->first ? 'selected' :''}}selected"
              ...
              <div class="preview">
                <div class="item rounded-lg h-full overflow-hidden">
                  <img
                    src="{{ $products->galleries()->exists() ? Storage::url($products->galleries->first()->url) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII='}}"
                    alt="front"
                    class="object-cover w-full h-full rounded-lg"
                  />
                </div>
              ...
              <div class="px-3 flex-none" style="width: 320px">
                <div class="rounded-xl p-4 pb-8 relative bg-white">
                  <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                    <img
                    src="{{ $recommendations->galleries()->exists() ? Storage::url($recommendations->galleries->first()->url) : 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mN88B8AAsUB4ZtvXtIAAAAASUVORK5CYII='}}"
                      alt=""
                      class="w-full h-full object-cover object-center"
                    />
                  </div>
                  <h5 class="text-lg font-semibold mt-4">{{ $recommendations->name }}</h5>
                  <span class="">IDR {{ number_format($recommendations->price) }}</span>
                  <a href="{{ route('details',$recommendations->slug) }}" class="stretched-link">
                    <!-- fake children -->
                  </a>
                </div>
              </div>
